...
class TestCreateAccessToken:
    ...
    def test_token_expiry_time(self):
        ...
        expected_exp = datetime.now(timezone.utc) + timedelta(minutes=config["jwt_exp_minutes"])
...
class TestGetCurrentUser:
    ...
    def test_get_current_user_missing_username_raises_exception(self, mock_request):
        ...
        invalid_data = {"role": "user", "exp": datetime.now(timezone.utc) + timedelta(minutes=30)}
        token = jwt.encode(invalid_data, config["jwt_secret"], algorithm=config["jwt_algorithm"])
        mock_request.cookies = {"access_token": token}
    ...
    def test_get_current_user_missing_role_raises_exception(self, mock_request):
        ...
        invalid_data = {"sub": "testuser", "exp": datetime.now(timezone.utc) + timedelta(minutes=30)}
        token = jwt.encode(invalid_data, config["jwt_secret"], algorithm=config["jwt_algorithm"])
        mock_request.cookies = {"access_token": token}
...
class TestRequireRole:
    ...
    def test_require_role_user_fails_for_different_role(self, mock_request):
        """Test that user role check fails for admin trying to access user-only endpoint."""
        token = create_test_token("adminuser", "admin")
        mock_request.cookies = {"access_token": token}
