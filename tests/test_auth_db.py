...
from app.auth.db import get_db, init_db, pwd_context, engine
...
def refresh_db():
    """Refresh the database for each test: drop all tables, recreate them, and initialize default admin."""
...
def test_user(test_db):
    ...
    user = User(username="testuser", password=pwd_context.hash("password"), role=UserRole.user)
    test_db.add(user)
    test_db.commit()
...
class TestInitDb:
    ...
    def test_init_db_creates_tables(self, test_db):
        ...
        tables = test_db.execute(text("SELECT name FROM sqlite_master WHERE type='table';")).fetchall()
...
class TestGetDb:
    ...
    def test_get_db_session_isolation(self):
        ...
        try:
            ...
            # Should not be visible in second session until commit/refresh
            user_in_db2 = db2.query(User).filter(User.username == "session1user").first()
