...
from app.auth.db import get_db, init_db, pwd_context
...
def create_user(
    ...
    current_user: dict = Depends(utils.require_role("admin")),
):
...
def login(login_data: LoginRequest, request: Request, db: Session = Depends(get_db)):
    ...
    if token:
        try:
            ...
            if current_user:
                raise HTTPException(
                    status_code=400, detail=f"Already logged in as {current_user}"
                )
...
# User logout
@auth_router.post(
    "/logout", summary="Logout user", description="Clear JWT cookie to logout"
)
def logout(current_user: dict = Depends(utils.get_current_user)):
    response = JSONResponse(
        content={"msg": f"User {current_user['username']} logged out"}
    )
